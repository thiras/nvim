...
- **`lua/plugins/image.lua`** sets `processor = "magick_cli"` and `build = false` to avoid building the magick rock (see image.nvim issue #91). Requires the `magick` CLI on PATH. Also sets `window_overlap_clear_enabled = true` — without it, Kitty-protocol images stay drawn on the graphics layer when floats (e.g. the snacks lazygit terminal) cover them. Note also that `build` belongs at the **top level** of the spec, not inside `opts` — nesting it under `opts` makes lazy.nvim ignore it and silently try to build the rock.
